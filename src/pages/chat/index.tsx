...
function Chat() {
  ...
  // ============ 【核心改造开始】============
  // 1. 从你自己的安全后端获取配置，替换原来的本地 config
  let safeConfig;
  try {
    const response = await fetch('https://1391127892-2azes7c6t6.ap-shanghai.tencentscf.com/api/config'); // 你的后端地址
    if (!response.ok) {
      throw new Error(`后端请求失败: ${response.status}`);
    }
    safeConfig = await response.json();
    console.log('从安全后端获取配置成功');
  } catch (error) {
    console.error('获取安全配置失败，请确保后端服务已启动:', error);
    throw new Error('无法从服务器获取必要配置，请稍后重试。');
  }

  // 2. 验证获取到的关键配置
  if (!safeConfig?.token) {
    throw new Error('后端返回的配置缺少 token');
  }
  if (!safeConfig?.botId) {
    throw new Error('后端返回的配置缺少 botId');
  }
  // ============ 【核心改造结束】============
  ...
  // 3. 初始化客户端，关键参数改用从后端获取的 safeConfig
  const client = new WsChatClient({
    token: safeConfig.token,           // 使用安全后端返回的 token
    baseWsURL: config.getBaseWsUrl(),  // 这个可以是前端配置，或也来自后端
    allowPersonalAccessTokenInBrowser: true,
    botId: safeConfig.botId,           // 使用安全后端返回的 botId
    debug: audioConfig?.debug,
    voiceId: safeConfig.voiceId || config.getVoiceId(), // 可优先使用后端配置的音色
