import {
  axios_default,
  reconnecting_websocket_mjs_default,
  require_crypto,
...
function isPlainObject(obj) {
  ...
  if (null === proto)
    return true;
  let baseProto = proto;
  while (null !== Object.getPrototypeOf(baseProto))
...
var APIError = class _APIError extends CozeError {
  static makeMessage(status, errorBody, message, headers) {
    ...
    if (errorBody) {
      ...
      if (msg)
        list.push(`msg: ${msg}`);
      if ((null == error ? void 0 : error.detail) && msg !== error.detail)
  ...
  static generate(status, errorResponse, message, headers) {
    ...
    if (400 === status || (null == error ? void 0 : error.code) === 4e3)
      return new BadRequestError(status, error, message, headers);
    if (401 === status || (null == error ? void 0 : error.code) === 4100)
      return new AuthenticationError(status, error, message, headers);
    if (403 === status || (null == error ? void 0 : error.code) === 4101)
      return new PermissionDeniedError(status, error, message, headers);
    if (404 === status || (null == error ? void 0 : error.code) === 4200)
      return new NotFoundError(status, error, message, headers);
    if (429 === status || (null == error ? void 0 : error.code) === 4013)
  ...
  */
  async createAndPoll(params, options) {
    ...
    while (true) {
      ...
      if ("completed" === chat.status || "failed" === chat.status || "requires_action" === chat.status)
        break;
    }
  ...
  */
  async *stream(params, options) {
    ...
    for await (const message of result)
      try {
        if ("Done" === message.event)
          yield new WorkflowEvent(Number(message.id), "Done");
        else
...
var WorkflowChat = class extends APIResource {
  ...
  */
  async *stream(params, options) {
    ...
    const result = await this._client.post(apiUrl, payload, true, options);
    for await (const message of result)
      if (message.event === chat_ChatEventType.DONE) {
        const ret = {
          event: message.event,
          data: "[DONE]"
        };
...
var getUniAppClientUserAgent = () => {
  if (!(null == uni ? void 0 : uni.getSystemInfoSync))
...
var handleError = (error) => {
  if (!error.isAxiosError && (!error.code || !error.message))
    return APIError.generate(500, void 0, `Unexpected error: ${error.message}`, void 0);
...
async function fetchAPI(url) {
  ...
  if (options.isStreaming && isAxiosStatic(axiosInstance)) {
    ...
    if (!axiosVersion || compareVersions(axiosVersion, "1.7.1") < 0)
      throw new CozeError("Streaming requests require axios version 1.7.1 or higher. Please upgrade your axios version.");
  }
  const checkError = () => {
...
new CozeAPI({
  ...
  const response = await axiosInstance({
    ...
    if ((null == error ? void 0 : error.status) === 401)
      checkError();
          ...
          if (done) {
            if (buffer) {
              ...
              try {
                ...
                if ((null == error ? void 0 : error.code) === 4101)
                  checkError();
              } catch (e) {
              }
              yield fieldValues;
    ...
    try {
      ...
      if (isStream) {
        if (contentType && contentType.includes("application/json")) {
          ...
          if (0 !== code && void 0 !== code)
            throw APIError.generate(response.status, result, msg, response.headers);
        }
        return stream();
      }
      if (!(contentType && contentType.includes("application/json")))
...
var getJWTToken = async (config, options) => {
  ...
  if (!keyFormat)
    throw APIError.generate(400, void 0, "Invalid private key format. Expected PEM format (RSA or PKCS8)", void 0);
